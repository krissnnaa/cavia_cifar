...
class MiniImagenet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        support_y = np.array([self.img2label[item.split('--')[0]]  # filename: n0153282900000005.jpg, first 9 chars are label
                              for item in self.support_x_batch[index]]).astype(np.int32)

        filenames_query_x = [item for item in self.query_x_batch[index]]
        query_y = np.array([self.img2label[item.split('--')[0]] for item in self.query_x_batch[index]]).astype(np.int32)
        ...
        for i, filename in enumerate(filenames_support_x):
            filename_preprocessed = filename[:-1] + '_preprocessed_{}'.format(self.imsize)
            path_preprocessed = os.path.join(self.path_preprocessed, filename.split('--')[0], filename_preprocessed)
            if not os.path.exists(path_preprocessed + '.npy'):
                if not os.path.exists(os.path.join(self.path_preprocessed, filename.split('--')[0])):
                    os.mkdir(os.path.join(self.path_preprocessed, filename.split('--')[0]))
                if self.subfolder_split:
                    support_x[i] = self.transform(os.path.join(self.path_images, filename.split('--')[0], filename))
                else:
        ...
        for i, filename in enumerate(filenames_query_x):
            ...
            if not os.path.exists(path_preprocessed + '.npy'):
                if not os.path.exists(os.path.join(self.path_preprocessed, filename.split('--')[0])):
                    os.mkdir(os.path.join(self.path_preprocessed, filename.split('--')[0]))
                if self.subfolder_split:
                    query_x[i] = self.transform(os.path.join(self.path_images, filename.split('--')[0], filename))
                else:
